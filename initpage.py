...
class InitPage(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        l2=tk.Label(self, text="Please input your online Smart Things' names: (Split by ',' and no space between words, ex:Thing01,Thing02)")
        l2.place(x=10, y=50)
    ...
    def spawnthread(self,controller):
        ...
        if ss_name == '' or thing_name == '' or thing_ip == '':
            ...
            for name in thing_name:
                if name not in temp_things:
                    ...
                    tk.messagebox.showinfo('Error', 'Please check your thing names! Cannot have two things with the same name!')
            ...
            if input_appropriate == True:
                ...
                for ip in thing_ip:
                    if ip not in temp_ips:
                        ...
                        tk.messagebox.showinfo('Error', 'Please check your ip addresses! Cannot have two things with the same ip!')
                        input_appropriate = False
                        break
                    temp_ip = ip.split('.')
...
class ThreadedClient(threading.Thread):
    ...
    def run(self):
        ...
        f = open('thing.csv', 'w')
        for thingId in owner:
            for i in range(num):
                if thingId == thing_name[i]:
                    f.write(thingId+','+thing_ip[i]+'\n')
        f.close()
        ...
        for thing in relationshipInfo:
            if thing:
                for relationship in thing:
                    f.write(relationship[0] + ','+ relationship[1]+ ',' + relationship[2] + ',' + relationship[3] + '\n')
        f.close()
