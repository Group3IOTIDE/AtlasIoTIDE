...
class Recipe(Frame):
    def __init__(self, parent, controller):
        ...
        '''buffer={}
        for row in rows:
            buffer[row[1]]=buffer.get(row[1],[])+[row[0]]'''

        # 表格来了
        # total_rows=buffer.keys()
        # t=Table(root)
        ...
        #roll.place(x=400,y=0,width=30,height=300)
        roll1.place(relx=0.91, rely=0.02, relwidth=0.08, relheight=0.95)
        treeview_service.configure(yscrollcommand=roll1.set)

        # treeview_service.pack(side=LEFT, fill=BOTH)
        '''for i in range(len(service_col)):
            treeview_service.insert('', i, values=(service_col[i]))'''
        ...
        # treeview_relationship.pack()
        '''for i in range(len(relationship_col)):
            treeview_relationship.insert('', i, values=(relationship_col[i]))'''

        # 下拉框选择过滤
        comboxlist0 = ttk.Combobox(self)  # 初始化
        id = [row[0] for row in self.rows]
        comboxlist0["values"] = ["default"] + id
        comboxlist0.current(0)
        comboxlist0.place(x=600, y=15)
        comboxlist0.bind("<<ComboboxSelected>>", lambda event: self.add_service(treeview_service, comboxlist0))  # 绑定事件,(下拉列表框被选中时，绑定filter()函数)

        comboxlist1 = ttk.Combobox(self)  # 初始化
        id = [row[0] for row in rows_rela]
        comboxlist1["values"] = ["default"] + id
        comboxlist1.current(0)
        comboxlist1.place(x=600, y=60)
        comboxlist1.bind("<<ComboboxSelected>>", lambda event: self.add_relationship(treeview_relationship, comboxlist1))  # 绑定事件,(下拉列表框被选中时，绑定filter()函数)
            ...
            for row in self.rows:
                if row[0] == comb0.get():
                    if row[1] not in exist_thing:
                        exist_thing += [row[1]]
                    #else:
                        #tkinter.messagebox.showinfo('Warning', 'Please make sure the services in the same thing won\'t be activated in the same time!')
                    service_col += [comb0.get()]
                    tv_service.insert('', len(service_col) - 1, values=(service_col[len(service_col) - 1]))
                    break
        else:
            tkinter.messagebox.showinfo('Error', 'Please select an appropriat item!')
        #else:
            #tkinter.messagebox.showinfo('Error', 'Can not add same item!')
    ...
    def add_relationship(self, tv_reltship, comb1):
        ...
        if comb1.get() not in relationship_col and comb1.get() != "default":
            ...
            tkinter.messagebox.showinfo('Error', 'Please select an appropriate item!')
        else:
            tkinter.messagebox.showinfo('Error', 'Can not add same item!')
